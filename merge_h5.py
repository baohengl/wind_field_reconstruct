...
def process_case(
    g_case_in: h5py.Group, g_case_out: h5py.Group,
    time_chunk: int = 8,
    ...
    # create outputs
    ds_sec = g_case_out.create_dataset(
        "section", shape=(T, Z, Ny, Nx, 3), dtype=OUT_DTYPE,
        chunks=(min(time_chunk, T), 1, Ny, Nx, 3), **COMP
    )
    g_case_out.create_dataset("z_values_section", data=z_vals, dtype=META_DTYPE)

    ds_prb = g_case_out.create_dataset(
        "probes", shape=(T, Z, Nprobe, 3), dtype=OUT_DTYPE,
        chunks=(min(time_chunk, T), 1, Nprobe, 3), **COMP
    )
    ...
    for t0 in range(0, T, time_chunk):
        ...
        # write to disk as float16
        ds_sec[t0:t1, ...] = buf_sec.astype(OUT_DTYPE, copy=False)
        ds_prb[t0:t1, ...] = buf_prb.astype(OUT_DTYPE, copy=False)

    return time  # return per-case time for global check
        ...
        for fpath in files:
            with h5py.File(fpath, "r") as h5_in:
                for case_name in h5_in.keys():
                    ...
                    # Make sure it's a group with coords/time/steps
                    if not isinstance(g_case_in, h5py.Group):
                        continue
                    ...
                    time_arr = process_case(
                        g_case_in, g_case_out,
                        time_chunk=time_chunk,
                        probe_x=probe_x, probe_y_list=probe_y_list,
                        idw_k=idw_k, idw_power=idw_power,
                    )
                    ...
                    if time_arr is not None and time_arr.size > 0:
                        if global_time is None:
                            ...
                            if (global_time.shape != time_arr.shape) or (not np.allclose(global_time, time_arr, rtol=1e-7, atol=1e-10)):
