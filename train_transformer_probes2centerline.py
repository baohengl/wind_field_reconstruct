...
class P2CDataset(Dataset):
    ...
    def __init__(self, h5:str, time_window:int=21, z_start_1based:int=21,
        ...
        with h5py.File(h5,"r") as f:
            ...
            for cname,g in f["cases"].items():
                ...
                # 基本形状检查：Ny=49, X=41, probes=9, C=3（若不同也允许，但会检查列数覆盖）
                ...
                if self.x_start is None:
                    # 默认以几何中心为中线带：例如 Xs=41 时，取 [center-10 : center+10] 共21列
                    center = Xs//2
                    half_w = (self.nx_target-1)//2
                    xs, xe = center - half_w, center + half_w + 1  # [xs:xe)
                else:
                    xs = int(self.x_start)
    ...
    def __getitem__(self, i:int):
        ...
        if self.augment:
            # x 翻转：对目标 (Nx_target) 翻转，不影响 probes
            if random.random()<0.5: s = s[:, ::-1, :]
            # y 翻转：对目标与 probes 的 9 个方位进行一致翻转
...
class P2CTransformer(nn.Module):
    ...
    def __init__(self,time_window:int, ny:int=49, nx_target:int=21,
        ...
        # learned queries + learned query pos
...
@torch.no_grad()
def evaluate(model, loader, device, tag="Val", epoch:int=None, epochs:int=None, amp_dtype=None, amp_on:bool=False, wb=None, ny:int=49, nx:int=21):
...
def parse_args():
    ...
    p.add_argument("--h5", required=True); p.add_argument("--save-dir", default="./runs/probes2centerline")
    ...
    p.add_argument("--nx-target", type=int, default=21, help="number of X columns for the centerline band (default 21)")
    p.add_argument("--x-start", type=int, default=None, help="override start column (0-based). If None, take centered slice.")
    ...
    # model (已为 Nx=21 做了更合适的默认值；可按需覆盖)
    p.add_argument("--d-model", type=int, default=160); p.add_argument("--nhead", type=int, default=5)
    p.add_argument("--num-enc-layers", type=int, default=4); p.add_argument("--num-dec-layers", type=int, default=2)
    p.add_argument("--ffn-dim", type=int, default=640); p.add_argument("--dropout", type=float, default=0.1)
...
def main():
    ...
    # datasets
    d_train_full=P2CDataset(args.h5, args.time_window, args.z_start_1based,
                            nx_target=args.nx_target, x_start=args.x_start,
                            augment=True,  time_stride=args.time_stride)

    # 读取一次用于评估 denom 的 Ny/Nx（更稳妥）
    ny = d_train_full._ny
    nx = d_train_full.nx_target
    ...
    d_val_full = P2CDataset(args.h5, args.time_window, args.z_start_1based,
                            nx_target=args.nx_target, x_start=args.x_start,
    ...
    # model & opt
    model=P2CTransformer(args.time_window, ny=ny, nx_target=nx,
                         d_model=args.d_model, nhead=args.nhead,
                         enc_layers=args.num_enc_layers, dec_layers=args.num_dec_layers,
                         ffn=args.ffn_dim, drop=args.dropout, act=args.act).to(args.device)
    ...
    if args.eval_only:
        vm,va=evaluate(model, dl_val, args.device, tag="Val", amp_dtype=amp_dtype, amp_on=amp_on, wb=wb, ny=ny, nx=nx)
        tm,ta=evaluate(model, dl_test, args.device, tag="Test", amp_dtype=amp_dtype, amp_on=amp_on, wb=wb, ny=ny, nx=nx)
    ...
    for epoch in epoch_bar:
        ...
        vm, va = evaluate(model, dl_val, args.device, tag="Val", epoch=epoch, epochs=args.epochs,
                          amp_dtype=amp_dtype, amp_on=amp_on, wb=wb, ny=ny, nx=nx)
        ...
        if best_val is None or vm<best_val:
            best_val=vm
            save_ckpt(ckpt_best, model, optim, epoch, args, best=best_val, last_only=False)
            if wb is not None:
