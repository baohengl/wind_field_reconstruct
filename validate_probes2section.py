...
"""
Validate a trained Probes->Section Transformer on the validation split.
...
- Randomly samples N validation items and plots x/y/z components.
  Each component figure is a 1x3 triptych: [GT, Reconstructed, Error].

Usage
-----
python validate_probes2section.py \
  --h5 windfield_interpolation_all_cases.h5 \
  --ckpt runs/probes2section/checkpoint_best.pt \
  --out-dir viz_val --num-samples 5 --seed 2025 --device cuda:0

If you only have model_only.pt (no args inside), specify arch via CLI:
  --from-model-only \
  --time-window 21 --d-model 192 --nhead 6 \
  --num-enc-layers 3 --num-dec-layers 1 --ffn-dim 768 --dropout 0.1 --act gelu
"""
...
class P2SDataset(Dataset):
    def __init__(self, h5:str, time_window:int=21, z_start_1based:int=21,
        ...
        self.items=[]  # (case,z,t)
        self._h5=None  # will be set per-worker or attached
...
def _triptych(axs, gt, pred, title_prefix:str, cmap_main:str="viridis", cmap_err:str="coolwarm"):
    ...
    if lim == 0.0: lim = 1e-8  # avoid zero range
    ...
    for a in axs:
        a.set_xlabel("x (41)"); a.set_ylabel("y (49)")

def plot_one_sample(out_dir:str, meta:dict, gt:np.ndarray, pred:np.ndarray):
    """
    gt, pred: (49,41,3). Save three figures (Ux, Uy, Uz), each with [GT, Pred, Error].
    """
...
def build_val_subset(h5:str, time_window:int, z_start_1based:int, time_stride:int,
    ...
    d_full = P2SDataset(h5, time_window, z_start_1based, augment=False, time_stride=time_stride)
    N = len(d_full)
...
def load_model_from_ckpt(ckpt_path:str,
                         ...
                         time_window:int=21, d_model:int=192, nhead:int=6,
                         num_enc_layers:int=3, num_dec_layers:int=1,
                         ffn_dim:int=768, dropout:float=0.1, act:str="gelu"):
    """
    Returns: model(nn.Module), used_arch(dict)
    If ckpt has args: use them; otherwise fall back to provided arch args.
    """
    ...
    if args_in is not None:
        tw = int(args_in.get("time_window", time_window))
        dm = int(args_in.get("d_model", d_model))
        nh = int(args_in.get("nhead", nhead))
        ne = int(args_in.get("num_enc_layers", args_in.get("num-enc-layers", num_enc_layers)))
        nd = int(args_in.get("num_dec_layers", args_in.get("num-dec-layers", num_dec_layers)))
        ff = int(args_in.get("ffn_dim", ffn_dim))
        dr = float(args_in.get("dropout", dropout))
        ac = str(args_in.get("act", act))
    else:
        tw, dm, nh, ne, nd, ff, dr, ac = time_window, d_model, nhead, num_enc_layers, num_dec_layers, ffn_dim, dropout, act

    model = P2STransformer(tw, dm, nh, ne, nd, ff, dr, ac).to(device)
    ...
    if isinstance(ckpt, dict) and "model" in ckpt:
        ...
        # could be a bare state_dict saved by torch.save(model.state_dict())
    ...
    used_arch = dict(time_window=tw, d_model=dm, nhead=nh, num_enc_layers=ne,
                     num_dec_layers=nd, ffn_dim=ff, dropout=dr, act=ac)
    return model, used_arch

# ===================== CLI & Main =====================
def parse_args():
    p=argparse.ArgumentParser("Validate probes->section model with visualization on validation split")
    ...
    p.add_argument("--num-enc-layers", type=int, default=3)
    p.add_argument("--num-dec-layers", type=int, default=1)
    p.add_argument("--ffn-dim", type=int, default=768)
    ...
    p.add_argument("--out-dir", type=str, default="viz_val")
...
def main():
    ...
    # ---- 1) load model first, to get true arch/time_window from ckpt ----
    amp_on = (str(args.device).startswith("cuda"))
    amp_dtype = torch.bfloat16 if args.bf16 else torch.float16
    model, used_arch = load_model_from_ckpt(
        ckpt_path=args.ckpt,
        device=args.device,
    ...
    # If CLI time_window differs, override to match checkpoint
    ...
    print(f"[Arch] {used_arch}")

    # ---- 2) build validation subset with the aligned time_window ----
    d_val = build_val_subset(
        h5=args.h5,
        time_window=args.time_window,
    ...
    for i in tqdm(pick, desc="Visualizing"):
        ...
        y = item["target"].unsqueeze(0).to(args.device)      # (1, 49,41,3)
        ...
        pred = yhat[0].detach().cpu().numpy()
        plot_one_sample(args.out_dir, item["meta"], gt, pred)
